  ...
  return products.value.filter((product) => {
    ...
    const matchesStatus =
      selectedStatus.value !== null
        ? product.is_active === selectedStatus.value
        : true;

    return matchesSearch && matchesBrand && matchesCategory && matchesStatus;
  });
});
//caaopj nhật trạng thái
const toggleStatus = async (item) => {
  try {
    const newStatus = !item.is_active;
    console.log("newStatus gửi đi:", newStatus);

    await axios.put(`http://localhost:3000/products/${item.id}`, {
      is_active: newStatus
    });

    item.is_active = newStatus; // Cập nhật lại UI theo dạng "0"/"1"
